...
export function useRetrospect() {
  ...
  const isTemplatePickerOpen = useAppStore(
    (state) => state.isTemplatePickerOpen
  );
  const retrospectViewMode = useAppStore((state) => state.retrospectViewMode);
  const customRetrospectiveTemplates = useAppStore(
    (state) => state.customRetrospectiveTemplates
  );
  ...
  const setIsTemplatePickerOpen = useAppStore(
    (state) => state.setIsTemplatePickerOpen
  );
  const setRetrospectViewMode = useAppStore(
    (state) => state.setRetrospectViewMode
  );
  const setCustomRetrospectiveTemplates = useAppStore(
    (state) => state.setCustomRetrospectiveTemplates
  );
  ...
  const updateTemplateDropdownPosition = useCallback(() => {
    ...
    const top = Math.min(window.innerHeight - padding - 24, rect.bottom + 8);
        ...
        try {
          ...
          if (Array.isArray(parsed)) {
            setCustomRetrospectiveTemplates(
              parsed.filter(
                (item: RetrospectiveTemplate) => typeof item?.id === 'string'
              )
            );
          } else {
            setCustomRetrospectiveTemplates([]);
